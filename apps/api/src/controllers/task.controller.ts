...
async function createGithubBranch(
    ...
    try {
        ...
        if (baseRefRes.ok) {
            ...
            if (!defaultRefRes.ok) return null;
...
const appendTaskActivity = async (taskId: any, input: TaskActivityInput) => {
    if (!taskId || !input?.action || !input?.message) return;
    try {
...
const toTaskSocketPayload = (task: any) => {
    const obj = typeof task?.toObject === 'function' ? task.toObject() : task;
    return {
        _id: obj?._id,
...
const emitTaskEvent = (event: 'task:created' | 'task:updated', task: any) => {
    const payload = toTaskSocketPayload(task);
    if (!payload.projectId) return;
    emitToProject(payload.projectId.toString(), event, payload);
};
...
const pickVerifierByPendingPreference = async (pool: any[], currentTaskId?: string) => {
    if (!pool.length) return null;
    const verifierIds = pool
        .map((user: any) => user?._id)
        .filter(Boolean);
    if (!verifierIds.length) return pickRandom(pool);

    const pendingLoads = await Task.aggregate([
        {
            $match: {
                _id: currentTaskId ? { $ne: currentTaskId as any } : { $exists: true },
    ...
    const loadByVerifierId = new Map<string, number>();
    for (const row of pendingLoads) {
        const id = row?._id?.toString?.();
        if (id) loadByVerifierId.set(id, Number(row?.count || 0));
    }

    const withLoad = pool.map((user: any) => {
        const id = user?._id?.toString?.();
        const load = id ? (loadByVerifierId.get(id) || 0) : 0;
        return { user, load };
    });

    const allZero = withLoad.every((entry: any) => entry.load === 0);
    if (allZero) {
...
const extractUsernameFromLegacyBranch = (branch: string | undefined): string | null => {
    if (!branch) return null;
    const tasksMatch = branch.match(/^tasks\/([^/]+)\//);
    if (tasksMatch?.[1]) return tasksMatch[1];
...
async function getGithubBranchSha(
    token: string,
    owner: string,
    repo: string,
    branch: string
...
async function moveGithubBranch(
    token: string,
    owner: string,
    repo: string,
    fromBranch: string,
    ...
    try {
        if (!fromBranch || !toBranch || fromBranch === toBranch) return { ok: true };
...
export const getTaskActivityLogs = async (req: AuthRequest, res: Response) => {
    try {
        const task = await Task.findById(req.params.id).select('_id projectId activityLogs');
        if (!task) return res.status(404).json({ message: 'Task not found' });

        const allowed = await hasTaskAccess(task, req.user as any);
        if (!allowed) return res.status(403).json({ message: 'Not authorized' });

        const hydrated = await Task.findById(req.params.id)
            .select('activityLogs')
            .populate('activityLogs.actorId', 'firstName lastName email role')
            .lean();

        const logs = Array.isArray((hydrated as any)?.activityLogs)
            ? [...(hydrated as any).activityLogs].sort((a: any, b: any) => {
...
export const finishTaskWork = async (req: AuthRequest, res: Response) => {
    try {
        ...
        const selectedVerifier: any = await pickVerifierByPendingPreference(pool, task._id?.toString?.());
...
export const fixTaskBranch = async (req: AuthRequest, res: Response) => {
    try {
        if (!INTERNAL_ROLES.includes(req.user!.role)) {
        ...
        const actor = await User.findById(req.user!.userId).select('githubUsername');
