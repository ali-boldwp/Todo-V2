...
export function TaskPreviewDrawer({ isOpen, onClose, task }: TaskPreviewDrawerProps) {
  ...
  const assignee = mockTeamMembers.find(m => m._id === task.assigneeId?._id || m._id === task.assigneeId);
  const verifier = mockTeamMembers.find(m => m._id === task.verifierId?._id || m._id === task.verifierId);
