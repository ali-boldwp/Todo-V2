...
export function DashboardPage() {
  ...
  // Real data using React Query
  const { data: projects = mockProjects } = useQuery({ queryKey: ['dashboard-projects'], queryFn: getProjects, enabled: canSeeClarificationTasks });
  const { data: tasks = mockTasks } = useQuery({ queryKey: ['dashboard-tasks'], queryFn: () => getTasks(''), enabled: canSeeClarificationTasks });
  const { data: teamMembers = mockTeamMembers } = useQuery({ queryKey: ['dashboard-team-members'], queryFn: getTeamMembers, enabled: isAdmin });
  const { data: clients = mockClients } = useQuery({ queryKey: ['dashboard-clients'], queryFn: getClients, enabled: isAdmin });
  ...
  const activeTeamMembers = teamMembers.filter((m: any) => m.isActive).length;
