...
export function TeamPage() {
  ...
  const toggleCanVerifyMutation = useMutation({
    mutationFn: ({ id, canVerifyTasks }: { id: string; canVerifyTasks: boolean }) => updateTeamMember(id, { canVerifyTasks }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['teamMembers'] }),
    onError: (err: any) => alert(err.response?.data?.message || 'Failed to update verification status'),
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createTeamMember({
        ...
        canVerifyTasks: form.canVerifyTasks,
  ...
  const toggleMemberStatus = (id: string) => {
    const member = membersArray.find((m: any) => m._id === id);
    if (!member) return;
    toggleStatusMutation.mutate({ id, isActive: !member.isActive });
    setMenuOpen(null);
  };

  const toggleCanVerify = (id: string, checked: boolean) => {
    toggleCanVerifyMutation.mutate({ id, canVerifyTasks: checked });
  };
  ...
  const resetPassword = async (id: string) => {
    try {
      ...
      setPwModal({ isOpen: true, password: result.password });
      setMenuOpen(null);
    } catch (err: any) {
  ...
  const displayedMembers =
    testerFilter === 'eligible' ? membersArray.filter((m: any) => !!m.canVerifyTasks) : membersArray;
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {user?.role !== 'client' && (
          ...
          {toggleStatusMutation.isError && (
            <div className="p-2 bg-red-50 text-red-600 text-sm">Failed to update status</div>
          )}
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button
                    ...
                    className="flex-1 px-4 py-2.5 text-sm bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white font-bold rounded-lg shadow-md hover:shadow-lg transition-all disabled:opacity-50"
                  >
