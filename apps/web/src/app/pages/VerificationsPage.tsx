...
export function VerificationsPage() {
  ...
  const { data: mockTasks = [] } = useQuery({ queryKey: ['tasks', 'all'], queryFn: () => getTasks('' as any) });
              ...
              {latestApproved.map((task: any) => {
                ...
                return (
                  <div
                    ...
                    onClick={() => setSelectedTask(task)}
                    className="bg-white border-2 border-emerald-200 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all cursor-pointer"
                  >
