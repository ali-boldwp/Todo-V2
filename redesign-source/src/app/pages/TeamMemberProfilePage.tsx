...
import { mockTeamMembers, mockTasks, mockProjects } from '../data/mockData';
import {
  User,
  Mail,
  Phone,
...
export function TeamMemberProfilePage() {
  ...
  // Find team member
  const member = mockTeamMembers.find((m) => m._id === id);
  ...
  const stats = {
    ...
    totalHours: memberTasks.reduce((sum, t) => sum + (t.totalWorkedSeconds || 0), 0) / 3600,
    activeProjects: [...new Set(memberTasks.map((t) => t.projectId))].length,
  };
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl border-2 border-slate-200 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[...new Set(memberTasks.map((t) => t.projectId))].map((projectId) => {
                const project = mockProjects.find((p) => p._id === projectId);
                const projectTasks = memberTasks.filter((t) => t.projectId === projectId);
